...
public class ArgList implements Iterable<Arg<?>>
{
	 ...
	 * 
	 * @throws Exception
	 *             if called, use ArgList.instance() to get the instance of this
	 *             singleton class
	 */
	 ...
	 * 
	 * @param i
	 *            int that surves no purpous other than giving a different
	 *            signature from public constructor
	 */
	 ...
	 * 
	 * @param args
	 *            String[] of arguments passed by main()
	 * @throws Exception
	 *             if there are any issues processing any arguments
	 */
	public static void updateFromMainArgs(String[] args) throws Exception
	{
		for (int i = 0; i < args.length; i++) // for each String argument in
												// args
		{
			for (Arg<?> a : argList) // for each Arg in argList
			{
				if (a.matchesFlag(args[i])) // if the flag matches
				{
					...
					{
						((ToggleArg) a).processInlineArg(""); // handle inline
																// of toggle
					}
